...
def createW(fallgenes, fpgenes, edgesp, thrp):
    fogenes = [i for i in fallgenes if i not in fpgenes]
    W = np.identity(len(fallgenes))
    newedges = []
    for edge in edgesp:
        ab = edge.split("#")
...
mtnpatients = []
mtnuseind = []
with open(basedir+ctype+'_mutn', 'r') as f:
    for line in f:
        lineData = line.rstrip().split("\t")
        count += 1
        if count == 1:
            for j in range(1,len(lineData)):
                if lineData[j] in mtnsamples:
                    continue
                if lineData[j].split("-")[0] == "TCGA" and lineData[j][:-3] in mtnpatients:
                    continue
                valt = int(lineData[j].split("-")[-1])
                if lineData[j].split("-")[0] == "TCGA" and valt >= 10 and valt <= 14:
                    continue
                mtnsamples.append(lineData[j])
                mtnpatients.append(lineData[j][:-3])
                mtnuseind.append(j)           
            continue
        mtngenes.append(lineData[0])
        mtnt = np.array([lineData[i] for i in mtnuseind]).astype(np.float)
        if count == 2:
            mtn = mtnt[np.newaxis,:]
        else:
...
for j in range(setnum):
    #print("Setting : " + str(j))   
    W = createW(fallgenes, fpgenes, edgesp, thrp)
    hactivationval = np.random.choice(activationlist)
    h2activationval = np.random.choice(activationlist)
    ...
    h2kregularizerval = np.random.choice(regularizerlist)
    h2kregparamval = np.random.choice(regparamlist)
    ...
    for train_index1, test_index1 in skf2.split(X_trainall,event_observed_trainall):
        ...
        for i in range(setrunnum):
            ...
            model.add(Dense(numnodes, activation=h2activationval,
                          kernel_regularizer=h2kregularizerval(h2kregparamval)))                                  
    ...
    if valci > valcimax:
        ...
        h2activationmax = h2activationval
        ...
        h2kregularizermax = h2kregularizerval
        h2kregparammax = h2kregparamval
...
for i in range(runnum):
    model = Sequential()
    model.add(CustomConnected(numnodes, Wmax, input_dim=numnodes, activation=hactivationmax,
                                  kernel_regularizer=hkregularizermax(hkregparammax)))
    model.add(Dense(numnodes, activation=h2activationmax,
                          kernel_regularizer=h2kregularizermax(h2kregparammax)))                                   
